...
async def list_addresses(current_user: CurrentUser, db: DbSession) -> list[Address]:
    result = await db.execute(
        select(Address).where(Address.user_id == current_user.id, Address.is_active.is_(True))
    )
...
async def update_address(
    address_id: UUID, payload: AddressUpdate, current_user: CurrentUser, db: DbSession
) -> Address:
