  ...
  if (isLoading) {
    return (
      <div className="p-12 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 font-medium">{t('generic.tableFetching')}</p>
      </div>
    );
  }
                ...
                return (
                  ...
                  {activeColumns.map((col) => (
                    <td key={col.key} className="px-4 lg:px-6 py-4 text-slate-700 align-top">
                      {renderCell(item[col.key], schema.properties[col.key], t)}
                    </td>
                  ))}
...
const renderCell = (value: unknown, schema: PropertySchema, t: (key: string, options?: Record<string, unknown>) => string) => {
  ...
  if (schema.type === 'boolean' && typeof value === 'boolean') {
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${value ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
        {value ? t('generic.active') : t('generic.inactive')}
      </span>
    );
  }
